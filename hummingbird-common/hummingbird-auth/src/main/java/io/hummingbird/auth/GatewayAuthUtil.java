package io.hummingbird.auth;

import java.util.UUID;

import io.hummingbird.cache.RedisCacheService;

/**
 * 网关鉴权util (保证服务调用的安全性) <br>
 * 会生成一个token和一个次级token, 在token验证不通过时，还可以继续验证次级token
 * 
 * @author kong
 */
public class GatewayAuthUtil {
	...
	public static String getToken() {
		String currentToken = RedisCacheService.get(VAR_GATEWAY_AUTH_TOKEN_KEY);
		if (isEmpty(currentToken)) {
			refreshToken();
			currentToken = RedisCacheService.get(VAR_GATEWAY_AUTH_TOKEN_KEY);
		}
		return currentToken;
	}
	...
	public static String getLowToken() {
		return RedisCacheService.get(VAR_GATEWAY_AUTH_LOW_TOKEN_KEY);
	}
	...
	public static boolean verifyToken(String token) {
		...
		String currentTokenLow = RedisCacheService.get(VAR_GATEWAY_AUTH_LOW_TOKEN_KEY);
	...
	public static void refreshToken() {
		...
		if (isEmpty(currentToken) == false) {
			RedisCacheService.set(VAR_GATEWAY_AUTH_LOW_TOKEN_KEY, currentToken);
		}
		// s2. 再刷新当前token
		RedisCacheService.set(VAR_GATEWAY_AUTH_TOKEN_KEY, createToken());
		RedisCacheService.set(VAR_GATEWAY_AUTH_TOKEN_REFRESH_LAST_TIME_KEY, String.valueOf(System.currentTimeMillis()));
	}
